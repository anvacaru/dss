...
contract DaiTest is DSTest {
    ...
    function testSetupPrecondition() public {
        Assert.equal(token.balanceOf(self), initialBalanceThis, "");
    }
    ...
    function testAllowanceStartsAtZero() public logs_gas {
        Assert.equal(token.allowance(user1, user2), 0, "");
    }
    ...
    function testValidTransfers() public logs_gas {
        ...
        Assert.equal(token.balanceOf(user2), sentAmount, "");
        Assert.equal(token.balanceOf(self), initialBalanceThis - sentAmount, "");
    }
    ...
    function testApproveSetsAllowance() public logs_gas {
        ...
        Assert.equal(token.allowance(self, user2), 25, "");
    }

    function testChargesAmountApproved() public logs_gas {
        uint amountApproved = 20;
        token.approve(user2, amountApproved);
        Assert.isTrue(TokenUser(user2).doTransferFrom(self, user2, amountApproved), "");
        Assert.equal(token.balanceOf(self), initialBalanceThis - amountApproved, "");
    }
    ...
    function testTransferFromSelf() public {
        ...
        Assert.equal(token.balanceOf(user1), 50, "");
    }
    ...
    function testMintself() public {
        ...
        Assert.equal(token.balanceOf(self), initialBalanceThis + mintAmount, "");
    }
    function testMintGuy() public {
        uint mintAmount = 10;
        token.mint(user1, mintAmount);
        Assert.equal(token.balanceOf(user1), mintAmount, "");
    }
    ...
    function testBurn() public {
        ...
        Assert.equal(token.totalSupply(), initialBalanceThis + initialBalanceCal - burnAmount, "");
    }
    function testBurnself() public {
        uint burnAmount = 10;
        token.burn(address(this), burnAmount);
        Assert.equal(token.balanceOf(self), initialBalanceThis - burnAmount, "");
    }
    function testBurnGuyWithTrust() public {
        uint burnAmount = 10;
        token.transfer(user1, burnAmount);
        Assert.equal(token.balanceOf(user1), burnAmount, "");

        TokenUser(user1).doApprove(self);
        token.burn(user1, burnAmount);
        Assert.equal(token.balanceOf(user1), 0, "");
    }
    ...
    function testTrustedTransferFrom() public {
        ...
        TokenUser(user1).doTransferFrom(self, user2, 200);
        Assert.equal(token.balanceOf(user2), 200, "");
    }
    function testApproveWillModifyAllowance() public {
        Assert.equal(token.allowance(self, user1), 0, "");
        Assert.equal(token.balanceOf(user1), 0, "");
        token.approve(user1, 1000);
        Assert.equal(token.allowance(self, user1), 1000, "");
        TokenUser(user1).doTransferFrom(self, user1, 500);
        Assert.equal(token.balanceOf(user1), 500, "");
        Assert.equal(token.allowance(self, user1), 500, "");
    }
    function testApproveWillNotModifyAllowance() public {
        Assert.equal(token.allowance(self, user1), 0, "");
        Assert.equal(token.balanceOf(user1), 0, "");
        token.approve(user1, uint(-1));
        Assert.equal(token.allowance(self, user1), uint(-1), "");
        TokenUser(user1).doTransferFrom(self, user1, 1000);
        Assert.equal(token.balanceOf(user1), 1000, "");
        Assert.equal(token.allowance(self, user1), uint(-1), "");
    }
    function testDaiAddress() public {
        //The dai address generated by hevm
        //used for signature generation testing
        Assert.equal(address(token), address(0xE58d97b6622134C0436d60daeE7FBB8b965D9713), "");
    }

    function testTypehash() public {
        Assert.equal(token.PERMIT_TYPEHASH(), 0xea2aa0a1be11a07ed86d755c93467f4f82362b452371d1ba94d1715123511acb, "");
    }

    function testDomain_Separator() public {
        Assert.equal(token.DOMAIN_SEPARATOR(), 0xc8bf33c5645588f50a4ef57b0c7959b26b61f1456241cc11261acabb2e7217d9, "");
    }

    function testPermit() public {
        Assert.equal(token.nonces(cal), 0, "");
        Assert.equal(token.allowance(cal, del), 0, "");
        token.permit(cal, del, 0, 0, true, v, r, s);
        Assert.equal(token.allowance(cal, del),uint(-1), "");
        Assert.equal(token.nonces(cal),1, "");
    }
    ...
    function testFailPermitWithExpiry() public {
        Assert.equal(now, 604411200 + 2 hours, "");
