...
contract VowTest is DSTest {
    ...
    function test_change_flap_flop() public {
        ...
        Assert.equal(vat.can(address(vow), address(flap)), 1, "");
        Assert.equal(vat.can(address(vow), address(newFlap)), 0, "");

        vow.file('flapper', address(newFlap));
        vow.file('flopper', address(newFlop));

        Assert.equal(address(vow.flapper()), address(newFlap), "");
        Assert.equal(address(vow.flopper()), address(newFlop), "");

        Assert.equal(vat.can(address(vow), address(flap)), 0, "");
        Assert.equal(vat.can(address(vow), address(newFlap)), 1, "");
    }

    function test_flog_wait() public {
        Assert.equal(vow.wait(), 0, "");
        vow.file('wait', uint(100 seconds));
        Assert.equal(vow.wait(), 100 seconds, "");

        uint tic = now;                                                                                                                                                       
        vow.fess(100 ether);                                                     
        Assert.isTrue(!try_flog(tic) , "");                                             
        Assert.isTrue( try_flog(tic) , ""); 
    }

    function test_no_reflop() public {
        flog(100 ether);
        Assert.isTrue( can_flop() , "");
        vow.flop();
        Assert.isTrue(!can_flop() , "");
    }
    ...
    function test_multiple_flop_dents() public {
        ...
        vat.mint(address(this), 100 ether);
        Assert.isTrue(try_dent(id, 2 ether,  rad(100 ether)), "");

        vat.mint(address(this), 100 ether);
        Assert.isTrue(try_dent(id, 1 ether,  rad(100 ether)), "");
    }
}
