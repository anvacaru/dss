...
import "../src/ds-value/src/value.sol";
...
contract EndTest is DSTest {
    ...
    function test_cage_basic() public {
        Assert.equal(end.live(), 1, "");
        Assert.equal(vat.live(), 1, "");
        Assert.equal(cat.live(), 1, "");
        Assert.equal(vow.live(), 1, "");
        Assert.equal(pot.live(), 1, "");
        Assert.equal(vow.flopper().live(), 1, "");
        Assert.equal(vow.flapper().live(), 1, "");
        end.cage();
        Assert.equal(end.live(), 0, "");
        Assert.equal(vat.live(), 0, "");
        Assert.equal(cat.live(), 0, "");
        Assert.equal(vow.live(), 0, "");
        Assert.equal(pot.live(), 0, "");
        Assert.equal(vow.flopper().live(), 0, "");
        Assert.equal(vow.flapper().live(), 0, "");
    }
    ...
    function test_cage_pot_drip() public {
        ...
        Assert.equal(pot.live(), 0, "");
        Assert.equal(pot.dsr(), 10 ** 27, "");
        Assert.isTrue(!try_pot_file("dsr", 10 ** 27 + 1), "");
    }
    ...
    function test_cage_collateralised() public {
        ...
        // global checks:
        Assert.equal(vat.debt(), rad(15 ether), "");
        Assert.equal(vat.vice(), 0, "");
        ...
        // local checks:
        Assert.equal(art("gold", urn1), 0, "");
        Assert.equal(ink("gold", urn1), 7 ether, "");
        Assert.equal(vat.sin(address(vow)), rad(15 ether), "");

        // global checks:
        Assert.equal(vat.debt(), rad(15 ether), "");
        Assert.equal(vat.vice(), rad(15 ether), "");

        // CDP closing
        ali.free("gold");
        Assert.equal(ink("gold", urn1), 0, "");
        Assert.equal(gem("gold", urn1), 7 ether, "");
        ali.exit(gold.gemA, address(this), 7 ether);

        end.thaw();
        end.flow("gold");
        Assert.isTrue(end.fix("gold") != 0, "");
        ...
        // global checks:
        Assert.equal(vat.debt(), 0, "");
        Assert.equal(vat.vice(), 0, "");

        ali.cash("gold", 15 ether);

        // local checks:
        Assert.equal(dai(urn1), 0, "");
        Assert.equal(gem("gold", urn1), 3 ether, "");
        ali.exit(gold.gemA, address(this), 3 ether);

        Assert.equal(gem("gold", address(end)), 0, "");
        Assert.equal(balanceOf("gold", address(gold.gemA)), 0, "");
    }
    ...
    function test_cage_undercollateralised() public {
        ...
        // global checks:
        Assert.equal(vat.debt(), rad(18 ether), "");
        Assert.equal(vat.vice(), 0, "");
        ...
        // local checks
        Assert.equal(art("gold", urn1), 0, "");
        Assert.equal(ink("gold", urn1), 2.5 ether, "");
        Assert.equal(art("gold", urn2), 0, "");
        Assert.equal(ink("gold", urn2), 0, "");
        Assert.equal(vat.sin(address(vow)), rad(18 ether), "");

        // global checks
        Assert.equal(vat.debt(), rad(18 ether), "");
        Assert.equal(vat.vice(), rad(18 ether), "");

        // CDP closing
        ali.free("gold");
        Assert.equal(ink("gold", urn1), 0, "");
        Assert.equal(gem("gold", urn1), 2.5 ether, "");
        ...
        // global checks:
        Assert.equal(vat.debt(), rad(3 ether), "");
        Assert.equal(vat.vice(), rad(3 ether), "");
        ...
        Assert.equal(gem("gold", urn1), rmul(fix, 15 ether), "");
        ...
        // local checks:
        Assert.equal(dai(urn2), 0, "");
        Assert.equal(gem("gold", urn2), rmul(fix, 3 ether), "");
        bob.exit(gold.gemA, address(this), rmul(fix, 3 ether));

        // some dust remains in the End because of rounding:
        Assert.equal(gem("gold", address(end)), 1, "");
        Assert.equal(balanceOf("gold", address(gold.gemA)), 1, "");
    }
    ...
    function test_cage_skip() public {
        ...
        Assert.equal(vat.vice(), rad(15 ether), "");    // now there is sin
        // get 1 dai from ali
        ...
        Assert.equal(dai(urn1), 14 ether, "");
        ...
        Assert.equal(dai(address(this)), 1 ether, "");       // bid refunded
        ...
        Assert.equal(art("gold", urn1), 0, "");
        Assert.equal(ink("gold", urn1), 7 ether, "");
        Assert.equal(vat.sin(address(vow)), rad(30 ether), "");
    ...
    function test_cage_collateralised_deficit() public {
        ...
        // global checks:
        Assert.equal(vat.debt(), rad(16 ether), "");
        Assert.equal(vat.vice(), rad(1 ether), "");
        ...
        Assert.equal(art("gold", urn1), 0, "");
        Assert.equal(ink("gold", urn1), 7 ether, "");
        Assert.equal(vat.sin(address(vow)), rad(16 ether), "");

        // global checks:
        Assert.equal(vat.debt(), rad(16 ether), "");
        Assert.equal(vat.vice(), rad(16 ether), "");
    ...
    function test_cage_undercollateralised_surplus() public {
        ...
        Assert.equal(dai(urn1), 0, "");
        uint256 fix = end.fix("gold");
        Assert.equal(gem("gold", urn1), rmul(fix, 14 ether), "");
    ...
    function test_cage_net_undercollateralised_multiple_ilks() public {
        ...
        Assert.equal(vat.debt(),             rad(20 ether), "");
        Assert.equal(vat.vice(),             rad(20 ether), "");
        Assert.equal(vat.sin(address(vow)),  rad(20 ether), "");

        Assert.equal(end.Art("gold"), 15 ether, "");
        Assert.equal(end.Art("coal"),  5 ether, "");

        Assert.equal(end.gap("gold"),  0.0 ether, "");
        Assert.equal(end.gap("coal"),  1.5 ether, "");
        ...
        Assert.equal(end.fix("gold"), ray(0.375 ether), "");
        Assert.equal(end.fix("coal"), ray(0.050 ether), "");

        Assert.equal(gem("gold", address(ali)), 0 ether, "");
        ali.pack(1 ether);
        ali.cash("gold", 1 ether);
        Assert.equal(gem("gold", address(ali)), 0.375 ether, "");

        bob.pack(1 ether);
        bob.cash("coal", 1 ether);
        Assert.equal(gem("coal", address(bob)), 0.05 ether, "");
        ...
        Assert.equal(gem("gold", address(ali)), 0.375 ether, "");
        Assert.equal(gem("coal", address(ali)), 0.05 ether, "");
        ...
        ali.pack(1 ether);
        ali.cash("gold", 1 ether);
        Assert.equal(end.out("gold", address(ali)), 3 ether, "");
        Assert.equal(end.out("coal", address(ali)), 1 ether, "");
        ali.pack(1 ether);
        ali.cash("coal", 1 ether);
        Assert.equal(end.out("gold", address(ali)), 3 ether, "");
        Assert.equal(end.out("coal", address(ali)), 2 ether, "");
        Assert.equal(gem("gold", address(ali)), 0.375 ether, "");
        Assert.equal(gem("coal", address(ali)), 0.05 ether, "");
    }
}
